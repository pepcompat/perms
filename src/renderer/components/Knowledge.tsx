...
import { Brain, Pencil, Trash2, Plus, Search, Bot, User } from 'lucide-react'
...
export default function Knowledge({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="flex h-[78vh] max-w-2xl flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Brain className="size-4 text-primary" /> คลังความรู้ AI
          </DialogTitle>
          <DialogDescription>สิ่งที่ AI จำได้ + ที่คุณสอน — ใช้ช่วยตอบในอนาคต</DialogDescription>
        </DialogHeader>
        ...
        <div className="-mr-2 flex-1 space-y-2 overflow-y-auto pr-2">
          {filtered.map((k) => (
            <div key={k.id} className="rounded-xl border border-border bg-background/40 p-3">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                {serverName(k.serverId) && (
                  <Badge variant="outline">🖥 {serverName(k.serverId)}</Badge>
                )}
                {k.tags.map((t) => (
          ...
          {filtered.length === 0 && (
            <div className="py-8 text-center text-xs text-muted-foreground">
              {items.length === 0 ? 'ยังไม่มีความรู้ — AI จะบันทึกเองเมื่อเรียนรู้ หรือเพิ่มเองด้านล่าง' : 'ไม่พบที่ค้นหา'}
            </div>
          )}
        </div>
